...
class Window(BaseWindow):
    ...
    def legacy_cleanup(self):
        ...
        if os.path.exists(const.LEGACY_DESKTOP_FILE):
            logger.debug(f"removing legacy autostart: {const.LEGACY_DESKTOP_FILE}")
        ...
        for action, pkg_type in const.QUEUE_PACKAGE_TYPES.items():
            ...
            for pkg in pkgs:
                if action == "do":
                    ...
                    rc, msgs = self.backend.AddTransaction(pkg.pkg_id, pkg_type)
                    if not rc:
    ...
    @common.exception_handler
    def _process_actions_installmode(self, action, package, always_yes, app_quit):
        ...
        if rc:
            ...
            dialogs.show_information(
                self,
                ngettext(
                    "Error in search for dependencies",
        ...
        try:
            ...
            if check:
                self.error_dialog.show(
                    ngettext(
                        "Can't remove protected package:",
                        "Can't remove protected packages:",
                        len(check),
                    )
