...
from yumex.common import _, check_dark_theme, format_block, is_url, pkg_id_to_full_name
...
class PackageDetails(GObject.GObject):
    __gsignals__ = {
        "info-changed": (GObject.SignalFlags.RUN_FIRST, None, (GObject.TYPE_STRING,))
    }

    VALUES = {0: "desc", 1: "updinfo", 2: "files", 3: "deps"}
    DEFAULT_STYLES = ["description", "filelist", "changelog", "changelog-header"]
    ...
    def get_style(self, tag_name):
        if tag_name in PackageDetails.DEFAULT_STYLES and check_dark_theme():
            tag_name += "_dark"
    ...
    def write(self, txt, style_name=None, newline=True):
        ...
        self._text.scroll_to_iter(self._buffer.get_end_iter(), 0.0, True, 0.0, 0.0)

    def clear(self):
        self._buffer.set_text("")

    def goto_top(self):
        self._text.scroll_to_iter(self._buffer.get_start_iter(), 0.0, False, 0.0, 0.0)
...
class PackageInfo(PackageDetails):
    ...
    def _show_updateinfo(self):
        ...
        if updinfo:
            ...
            if self._is_fedora_pkg():
                self.write(_("\nFedora Updates:"), "changelog-header", newline=True)
                url = const.FEDORA_PACKAGES_URL + self._get_name_for_url() + "/updates"
    ...
    def _show_changelog(self):
        ...
        if changelog:
            ...
            if self._is_fedora_pkg():
                self.write(_("\nOnline Changelog:"), "changelog-header", newline=True)
                url = (
                    const.FEDORA_PACKAGES_URL + self._get_name_for_url() + "/changelog"
                )
                self.add_url(url, url, newline=True)
